...
To test whether the moderate-depth rescue scales, we add a deeper held-out family, \texttt{mixed\_trig\_hidden\_apart}, built from three structural blocks with a guided chain of length 7:
\[
\begin{aligned}
...
The model-class scope is also limited. We evaluate \mlp{}, \staticKAN{}, and \hyperkan{} policy heads under a shared encoder to isolate policy-head and search-control effects. We do not claim that root-collapse is universal across all neural architectures; testing attention-based encoders and larger sequence models is an important next step. A lightweight Transformer-encoder diagnostic can address this concern only as a bounded check, not as a replacement for a systematic architecture study.
