...
def multi_task_loss(
    ...
    frontier_logits: Tensor | None = None,
    ...
    frontier_loss = torch.tensor(0.0, device=logits.device)
    ...
    if frontier_logits is not None and frontier_targets is not None and frontier_loss_weight != 0.0:
        frontier_loss = masked_bce_with_logits(frontier_logits, frontier_targets, mask=non_terminal)
    total = (
        action_loss
        ...
        + (frontier_loss_weight * frontier_loss)
    )
    return total, {
